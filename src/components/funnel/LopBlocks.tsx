        ...
        <div className="min-w-[900px]">
          ...
          >
            ...
            {SEGMENTS.map((segment) => (
              <div key={segment} className="px-4 py-2 text-center text-sm font-semibold text-slate-600 dark:text-slate-200">
          ...
          >
            ...
            {SEGMENTS.map((segment) => (
              <div key={segment} className="flex items-center justify-center bg-white/80 px-4 py-3 dark:bg-slate-950/40">
                <div className="text-base font-semibold text-slate-900 dark:text-slate-50">{formatValueM(data[segment].valueM)}</div>
              </div>
            ))}
          </div>
          ...
          >
            ...
            {SEGMENTS.map((segment) => (
              <div key={segment} className="flex items-center justify-center bg-white/80 px-4 py-3 dark:bg-slate-950/30">
                <div className="text-sm font-semibold text-blue-600 dark:text-blue-300">{formatPercent(data[segment].pctRkap)}</div>
              </div>
            ))}
          </div>
          ...
          >
            ...
            {SEGMENTS.map((segment) => (
              <div key={segment} className="flex items-center justify-center bg-white/80 px-4 py-3 dark:bg-slate-950/40">
                <div className="text-sm font-semibold text-emerald-600 dark:text-emerald-300">{formatPercent(data[segment].pctStg)}</div>
              </div>
            ))}
          </div>
        </div>
