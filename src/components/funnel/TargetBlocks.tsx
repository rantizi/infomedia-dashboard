...
function TargetRow({ label, labelColor, values }: TargetRowProps) {
  const labelClasses: Record<TargetRowProps["labelColor"], string> = {
    rkap:
      "bg-rose-100/60 border-rose-200/70 text-rose-800 dark:bg-rose-500/25 dark:border-rose-500/40 dark:text-rose-50",
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-center bg-white/80 px-4 py-3 dark:bg-slate-950/40">
        <div
          className={cn(
            "inline-flex items-center rounded-full border px-4 py-2 text-sm font-semibold backdrop-blur-sm shadow-sm",
      ...
      {SEGMENTS.map((segment) => (
        ...
        >
          <div className="text-base font-semibold text-slate-900 dark:text-slate-50">{formatValueM(values[segment])}</div>
        </div>
      ))}
    </div>
  );
